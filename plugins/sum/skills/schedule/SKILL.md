...
Recurring playbook runs, delivered by email. Helper: `../api/scripts/sum_api.py`. Clean operationIds exist: `list_schedules`, `create_schedule`, `show_schedule`, `update_schedule`, `delete_schedule`, `pause_schedule`, `resume_schedule`, `list_schedule_runs`, `run_schedule_now`.
...
1. Schedules target **playbooks** today (`kind: "playbook"`). Find the target: `call GET /v1/projects/<PID>/playbooks`. If the user has no playbook for what they want, say so — `/sum:report` is for one-off reports; a playbook must exist first.
...
- Pass `--profile <name>` when the user names an environment.
