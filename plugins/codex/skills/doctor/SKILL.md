...
If the user reports Summation MCP tools failing with auth errors while `doctor` passes: the MCP registration carries its own bearer header — re-run `$addison-login` (steps 1-4) to mint a fresh credential and re-register the server via `mcp-connect`.
...
- No credential found → hand off to the `login` skill.
