...
- Sample-row peeks stay at `limit ≤ 5`; this is discovery, not analysis (`/addison-internal:query` is for that).
- Suggest next steps: a `/addison-internal:query` against a found table, or `/addison-internal:report` for a full analysis.
