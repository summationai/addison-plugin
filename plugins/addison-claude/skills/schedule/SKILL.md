...
1. Schedules target **playbooks** today (`kind: "playbook"`). Find the target: `call GET /v1/projects/<PID>/playbooks`. If the user has no playbook for what they want, say so — `/addison:report` is for one-off reports; a playbook must exist first.
