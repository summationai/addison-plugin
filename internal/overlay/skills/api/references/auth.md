...
4. `~/.summation/summation-config-internal` (canonical).
5. Installed skill directory `.summation-config` (legacy).
6. Home directory `.summation-config` (legacy).

A config found only in a legacy location is copied to `~/.summation/summation-config-internal` on first use; the legacy file is left in place and the canonical path wins afterward.
...
On approval, the helper stores `SUM_API_DEVICE_LOGIN_CREDENTIAL` in `~/.summation/summation-config-internal`. Do not print or quote `device_code` in chat; the helper now keeps it only in temporary local polling state until `login-poll` finishes.
